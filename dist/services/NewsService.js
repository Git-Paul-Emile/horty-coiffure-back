...
class NewsService {
    newsRepository = newsRepository; //on réutilise l'instance
    async create(data) {
        try {
            ...
            const duplicate = existingNews.find(n => n.title.toLowerCase() === validatedData.titre.toLowerCase() && n.status !== 'ARCHIVED');
            if (duplicate) {
    ...
    async update(id, data) {
        try {
            ...
            if (validatedData.titre) {
                ...
                const duplicate = allNews.find(n => n.title.toLowerCase() === validatedData.titre.toLowerCase() &&
                    n.id !== id &&
                    n.status !== 'ARCHIVED');
                if (duplicate) {
                    throw new Error('Une actualité avec ce titre existe déjà');
            ...
            if (validatedData.archive !== undefined)
                dataToUpdate.status = validatedData.archive ? NewsStatus.ARCHIVED : NewsStatus.PUBLISHED;
