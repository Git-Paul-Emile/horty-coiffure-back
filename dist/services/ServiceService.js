...
class ServiceService {
    serviceRepository = serviceRepository; //on réutilise l'instance
    async create(data) {
        try {
            ...
            const duplicate = existingServices.find(s => s.name.toLowerCase() === validatedData.nom.toLowerCase() && s.status !== 'INACTIVE');
            if (duplicate) {
    ...
    async update(id, data) {
        try {
            ...
            if (validatedData.nom) {
                ...
                const duplicate = allServices.find(s => s.name.toLowerCase() === validatedData.nom.toLowerCase() &&
                    s.id !== id &&
                    s.status !== 'INACTIVE');
                if (duplicate) {
                    throw new Error('Un service avec ce nom existe déjà');
            ...
            if (validatedData.duree !== undefined)
                dataToUpdate.duration = validatedData.duree;
            ...
            if (validatedData.inclus !== undefined)
                dataToUpdate.included = validatedData.inclus;
            if (validatedData.exclus !== undefined)
                dataToUpdate.excluded = validatedData.exclus;
            if (validatedData.image !== undefined)
                dataToUpdate.imageUrl = validatedData.image;
            if (validatedData.variantes !== undefined)
                dataToUpdate.variants = validatedData.variantes;
