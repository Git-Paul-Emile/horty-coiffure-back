...
class AppointmentSettingsService {
   private appointmentSettingsRepository = appointmentSettingsRepository; //on réutilise l'instance




   async create(data: ParametresRendezVousCreate): Promise<ParametresRendezVous> {
       try {
   ...
   async findAll(): Promise<ParametresRendezVous[]> {
       try {
   ...
   async findById(id: string): Promise<ParametresRendezVous | null> {
       try {
   ...
   async update(id: string, data: ParametresRendezVousUpdate): Promise<ParametresRendezVous> {
       try {
           // Validation des données
           ...
           if (validatedData.urlCalendly !== undefined) dataToUpdate.calendlyUrl = validatedData.urlCalendly;
           if (validatedData.modeUrgence !== undefined) dataToUpdate.urgencyMode = validatedData.modeUrgence;
           if (validatedData.messageUrgence !== undefined) dataToUpdate.urgencyMessage = validatedData.messageUrgence;
   ...
   async delete(id: string): Promise<ParametresRendezVous> {
       try {
           // Vérifier si les paramètres existent
           const existingSettings = await this.appointmentSettingsRepository.findById(id);
           if (!existingSettings) {
