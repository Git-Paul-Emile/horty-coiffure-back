...
class CategoryService {
   private categoryRepository = categoryRepository; //on réutilise l'instance
   ...
   async create(data: CategorieCreate): Promise<Categorie> {
       try {
           ...
           const duplicate = existingCategories.find(c =>
               c.name.toLowerCase() === validatedData.nom.toLowerCase() && c.status !== 'INACTIVE'
           );
   ...
   async update(id: string, data: CategorieUpdate): Promise<Categorie> {
       try {
           // Validation des données
           ...
           if (validatedData.nom) {
               ...
               const duplicate = allCategories.find(c =>
                   c.name.toLowerCase() === validatedData.nom!.toLowerCase() &&
                   c.id !== id &&
                   c.status !== 'INACTIVE'
               );
           ...
           if (validatedData.categorieParent !== undefined) dataToUpdate.parentId = validatedData.categorieParent;
