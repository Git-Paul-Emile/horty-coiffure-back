...
class RealizationService {
   private realizationRepository = realizationRepository; //on réutilise l'instance




   async create(data: RealisationCreate): Promise<Realisation> {
       try {
   ...
   async findById(id: string): Promise<Realisation | null> {
       try {
   ...
   async update(id: string, data: RealisationUpdate): Promise<Realisation> {
       try {
           // Validation des données
           ...
           if (validatedData.legende !== undefined) dataToUpdate.caption = validatedData.legende;
   ...
   async delete(id: string): Promise<Realisation> {
       try {
           // Vérifier si la réalisation existe
           const existingRealization = await this.realizationRepository.findById(id);
           if (!existingRealization) {
