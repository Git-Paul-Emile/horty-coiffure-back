...
class FeedbackService {
   private feedbackRepository = feedbackRepository; //on réutilise l'instance




   async create(data: CommentaireCreate): Promise<Commentaire> {
       try {
   ...
   async findById(id: string): Promise<Commentaire | null> {
       try {
   ...
   async update(id: string, data: CommentaireUpdate): Promise<Commentaire> {
       try {
           // Validation des données
           ...
           if (validatedData.commentaire !== undefined) dataToUpdate.comment = validatedData.commentaire;
           if (validatedData.archive !== undefined) dataToUpdate.status = validatedData.archive ? FeedbackStatus.ARCHIVED : FeedbackStatus.READ;
   ...
   async delete(id: string): Promise<Commentaire> {
       try {
           // Vérifier si le commentaire existe
           const existingFeedback = await this.feedbackRepository.findById(id);
           if (!existingFeedback) {
